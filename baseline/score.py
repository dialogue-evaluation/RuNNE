...
def load_jsonl(in_path, subdir, mode): # Было добавлено лишь возможность сохранять и читать другие файлы помимо test.jsonl
    data = []
...
class Evaluator:
    ...
    def evaluate(self):
        ...
        summary_dict["Macro F1"] = sum(filtered_f1s) / float(len(filtered_f1s)) if len(filtered_f1s) > 0 else 0.
        summary_dict["Macro F1 few-shot"] = sum(
            [each for i, each in enumerate(f1_scores_list) if i in self.few_shot_type_inds]
        ) / float(len(self.few_shot_type_inds))
        print("Macro F1: {:.2f}%".format(summary_dict["Macro F1"] * 100))
        print("Macro F1 few-shot: {:.2f}%".format(summary_dict["Macro F1 few-shot"] * 100))
